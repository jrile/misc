...
import akka.testkit.{ TestProbe, ImplicitSender, TestKit, TestKitBase }
...
class BinaryTreeSuite(_system: ActorSystem) extends TestKitBase {
...
import BinarySearchTree._
  ...
  def verify(probe: TestProbe, ops: Seq[Operation], expected: Seq[OperationReply]): Unit = {
    val topNode = system.actorOf(Props[BinarySearchTree])
